...
function bindBtnsChoice() {
    ...
    if(element.classList.contains('btn-choice')) {
        ...
        response.push(options)
        chat.innerHTML += `<div class="well col-xs-10 col-sm-10 col-md-9 float-r">${element.textContent}</div>`
        responseArea.classList.remove('appear')
...
function userSubmit() {
    ...
    setTimeout(function() {
        ...
        const options = [
            {id: 1, apoio: '', text: 'Informações sobre um imóvel', momento: 'Como posso lhe ajudar?'},
            {id: 2, apoio: '', text: 'Interesse em comprar um imóvel', momento: 'Como posso lhe ajudar?'},
            {id: 3, apoio: '', text: 'Outras informações', momento: 'Como posso lhe ajudar?'}
        ]
        resetTyped(
            "#typed",
            [`Olá ${nome.value} tudo bem?<br>Eu sou a “VITÓRIA”. ^500 Meu papel é entender a sua necessidade para direcionar você ao melhor profissional especializado da minha equipe.<br>Preciso de no máximo 03 minutos do seu tempo, e não se preocupe, é seguro compartilhar suas informações comigo.`],
            options
        )
    },500)
